...
export async function replyMessage(contact, content) {
    ...
    try {
        ...
        const message = await retryRequest(() => getChatGPTReply(content, contactId), config.retryTimes, 500);
        ...
        if (e.message.includes('timed out')) {
            console.error('ERROR: Please try again, ChatGPT timed out for waiting response.');
        }
